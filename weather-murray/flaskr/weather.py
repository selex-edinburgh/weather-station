...
def hello():
        ...
        for i in range(0, 23):
            cur = g.db.execute("select sum(inches) from rainfall_values where timestamp > datetime('now', '-{} hours') and timestamp < datetime('now', '-{} hours')".format(i+1, i))
            val = cur.fetchone()[0]
            if val == None:
                        val = 0
        ...
        return render_template('main.html',entries=entries, rainfalltoday=rainfalltoday, lr=lr, rcvalues=rcvalues, \
                curtemp = curtemp, minmax=minmax, curpressure = curpressure, curwindspeed = curwindspeed, lightlevel=lightlevel, curwinddir=curwinddir )
