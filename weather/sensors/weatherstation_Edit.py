...
#Start all scripts
os.system("sudo python %s &" % ("weather/flaskr/weather_Edit.py"))
os.system("sudo python %s &" % ("Rainfall_Edit.py"))
os.system("sudo python %s &" % ("temperature-pressure.py"))
os.system("sudo python %s &" % ("anemometer.py"))
os.system("sudo python %s &" % ("LDR.py"))
os.system("sudo python %s &" % ("wind-direction.py"))
...
while (True):
        ...
        if "weather_Edit.py" not in tmp[:]:
                err[0] = 1
        else:
                err[0] = 0
        if "Rainfall_Edit.py" not in tmp[:]:
                err[1] = 1
        else:
                err[1] = 0
